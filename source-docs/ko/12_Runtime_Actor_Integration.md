...
Actor Blueprint에서는 `Request Actor`, `Source Actor`, `Target Actor`의 기본값이 `Self`입니다. 예시의 `Self` 연결은 흐름을 설명하기 위한 것이며 같은 Actor를 처리한다면 선을 생략할 수 있습니다. `Resume Alpha`와 `Apply Control Rotation`은 노드의 고급 핀을 펼쳐야 나타납니다. 전체 기본/숨김 핀 표는 [Blueprint 공개 API](11_User_API.md#21-blueprint-기본-핀-정책)를 참고하십시오.
