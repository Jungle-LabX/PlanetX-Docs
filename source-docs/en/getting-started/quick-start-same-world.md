...
> For an **External Level** setup in which Orbit and Ground use different Levels, see [Advanced Guide — Multi-Level Handoff](?lang=en&doc=quick-start-level-handoff) after this workflow succeeds.
...
Choose **Content Browser > Add > Miscellaneous > Planet Asset**. For this guide, use Planet ID `FirstPlanet`, Planet Radius `100 km`, the default Coordinate Convention, and save the Asset as `PA_FirstPlanet`. See [Create Your First Planet Asset](?lang=en&doc=create-first-planet) only if you need the setting concepts explained in more detail.

![Content Browser Add menu showing Add, Miscellaneous, and Planet Asset](../../images/quick-start-same-world/qs-02-create-planet-asset-menu.png)
...
![PlanetX Proxy Bake Editor command in the Unreal Editor Tools menu](../../images/quick-start-same-world/qs-05-open-proxy-bake.png)
...
![Successful GroundLevel source scan with PA_FirstPlanet, no unsupported or manual-review sources, and Bake in Editor enabled](../../images/quick-start-same-world/qs-07-scan-success.png)
...
![Preview settings with MI_PlanetX_Earth assigned and Apply and Build available](../../images/quick-start-same-world/qs-10-preview-settings.png)
...
![PlanetX Planet Actor Details with PA_FirstPlanet assigned and Auto Register Runtime enabled](../../images/quick-start-same-world/qs-12-planet-actor-details.png)
...
![BP_Player with PlanetX Coordinate and Viewpoint components referencing PlanetXPlanetActor and the GroundLevel Section](../../images/quick-start-same-world/qs-13-player-components.png)
...
> Do not try to assign a specific Level Actor from the Blueprint class defaults. A Blueprint class cannot hold a reference to an Actor instance in a particular Level. Projects that spawn the Pawn through GameMode must resolve and assign the Planet reference after spawn; see [Runtime Integration](?lang=en&doc=runtime-integration).
...
![PlanetX Mode Validate palette showing zero errors and zero warnings](../../images/quick-start-same-world/qs-14-diagnostics-ready.png)
...
![GroundLevel running in PIE with the completed PlanetX planet, Environment Manager, Transition Endpoint, and player actors](../../images/quick-start-same-world/qs-15-pie-result.png)
...
If the issue continues, see [Diagnostic Tools](?lang=en&doc=diagnostic-tools) and [Runtime Travel Troubleshooting](?lang=en&doc=runtime-travel-troubleshooting).
